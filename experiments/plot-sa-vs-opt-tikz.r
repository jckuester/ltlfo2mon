...
if(toPdf) {
  ...
  output_file_name <- 'experiments_sa_tikz'
  isStandalone <- FALSE
...
# read files
flist_progression <- list.files(path="results/", pattern = "10000_optimised.dat$",full.names = TRUE)
flist_SAbased <- list.files(path="results/", pattern = "10000_sa.dat$",full.names = TRUE)
...
for (i in c(1:numfiles)){  
  ...
  diff <- (data_SAbased$V3 + lookuptables_sa$V1[i]) - (data_progression$V3 + lookuptables_opt$V1[i])
...
data2.labels <- data.frame(
  ...
  value = c(13,50,13,100,10,5,7.4,40),
...
#  geom_boxplot(aes(x=factor(formula,levels=formula_file$formula),group=grp,ymin=min,ymax=max, lower = avg - 10, middle = avg, upper = avg + 10), size=0.05,width=0.1,position=position_dodge(width=5.1),color="dark gray")+
  ...
  theme(axis.title.x=element_blank(),
        axis.title.y=element_blank(),legend.position=c(.1, .1)) +
