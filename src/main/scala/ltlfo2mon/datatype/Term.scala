...
case class Funct(name: String, args: Vector[Term]) extends Term(name) {  
  def interpret(struct: Structure, v: Valuation) = struct.functs.get(name) match {
    case None => None
    case Some(funct) => Some(funct(args.map(_.interpret(struct, v))))
  }  
}

case class Const(name: String) extends Term(name) {
  override def toString = name
